...
func (m *MediaConnection) AddStream(tr *webrtc.TrackRemote) {
	m.Logger.Debug().Msgf("Receiving stream: %v", tr)
...
func (m *MediaConnection) HandleMessage(message *models.Message) error {
	...
	default:
		m.Logger.Warn().Msgf("Unrecognized message type:%s from peer:%s", mtype, m.peerID)
	}
	return nil
}
...
func (m *MediaConnection) Answer(tl webrtc.TrackLocal, options *AnswerOption) {
	...
	if m.localStream != nil {
		m.Logger.Warn().Msg("Local stream already exists on this MediaConnection. Are you answering a call twice?")
		return
	}
